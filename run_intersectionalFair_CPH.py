# -*- coding: utf-8 -*-
"""
Created on Mon Apr 27 00:29:01 2020
...
def criterionHinge_intersect(target_fairness,prediction,S, intersect_groups):
    zeroTerm = torch.tensor(0.0)
    model_fairness = intersect_fairness_Train(prediction,S, intersect_groups)
    return torch.max(zeroTerm, (model_fairness-target_fairness))
#%% intersectional fairness as reguralizer for training Cox model
def intersect_fairness_Train(prediction,S,intersect_groups):    
    ...
    h_ratio_group = torch.zeros(len(intersect_groups),dtype=torch.float)
    group_total = torch.zeros(len(intersect_groups),dtype=torch.float)
    ...
    epsilon = torch.tensor(0.0) # intersectional fairness
    for i in  range(len(avg_h_ratio_group)):
        for j in range(len(avg_h_ratio_group)):
            if i == j:
                continue
            else:
                epsilon = torch.max(epsilon,torch.abs(torch.log(avg_h_ratio_group[i])-torch.log(avg_h_ratio_group[j]))) 
    return epsilon  
...
import sys
sys.stdout=open("intersectional_fair_cox_ph_output.txt","w")

#%% define the model
lamda = torch.tensor(0.4) # chosen by grid search on dev set
...
for epoch in range(num_epochs):   
    ...
    # fairness constraint
    R_loss = criterionHinge_intersect(target_fairness,outputs,S_train, intersectionalGroups)
...
plt.grid(True)
plt.savefig('intersectFair_auc.png',dpi = 600)
...
S_age = S_test[:,0] # race is in the 1st column
...
#%% save the model
torch.save(coxPH_model.state_dict(), "trained-models/Intersectional_Fair_CoxPH")
